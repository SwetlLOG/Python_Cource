...
def printinfo(my_data):
    print('id \t Фамилия \t  Имя  \t  номер телефона')
    for elem in my_data:
        print(*elem, sep='\t')


def export_to_csv(data):
    with open('TelSpravka.csv', 'w', encoding='utf-8') as file:
...
def search(data):
    ...
    seached_data = [elem for elem in data if searching in elem]
...
def add_record(data):
    ...
    second_name = input('Введите имя нового человека: ')
    tel = input('Введите номер телефона: ')
    data.append((id, name, second_name, tel))
    return data
